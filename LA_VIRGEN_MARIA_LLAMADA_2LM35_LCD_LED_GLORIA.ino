...
TinyGPS gps;
SoftwareSerial nss(10, 11); // GPS 10(tx) 11 (rx)
SoftwareSerial mySerial(12, 13);// MODULO A7 LLAMADA TELEFONO
...
int led = 2; // LED 13 PLACA ARDUINO
...
void loop()
{
 ...
 //dataString =  SD_lat + "," + SD_lon + "," + celsius+ "," + celsius1;// ESCRIBE EN LA SD LATITUD TEMP MOTOR Y TEMP BATERIA
  ...
  //dataString = SD_lat + "," + SD_lon+ "," + celsius+ "," + celsius1;

  dataString = SD_lat + "," + SD_lon;
  
 if(SD_date_time != "invalid")
 
  //digitalWrite(led, HIGH); // FUNCIONANDO
  blinkSlowly();
 else
  //digitalWrite(led,LOW); //CUANDO NO FUNCIONA
  blinkQuickly();
  
// Abriendo el archivo CSV para grabar los datos
  File dataFile = SD.open("ruta.csv", FILE_WRITE);  //  ruta.csv EXCEL separado por comas     .txt
  if (dataFile)
  {
...
void blinkQuickly () // FUNCION PARPADEO DEL LED
{
...
void blinkSlowly () // FUNCION PARPADEO DEL LED
{
  digitalWrite(led,HIGH);
  delay(500);
  digitalWrite(led,LOW);
